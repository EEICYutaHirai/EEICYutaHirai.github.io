...
let hurusato_music = "note, millisecond\n";
...
function load() {
    ...
    var timingCallbacks = new ABCJS.TimingCallbacks(visualObj, {
        ...
        beatCallback: function (beatNumber, totalBeats, totalTime, position, debugInfo) {
            // positionは画面上の音符の場所
            // debugInfoは，
            //debugInfo.millisecondsに持続時間が入っている
            //hurusato.slice(debugInfo.startChar, debugInfo.endChar).replace(/[0-9]/g, "").replace(/\s/g, "").replace(/\(/g, "").replace(/\)/g, "")
            // console.log(beatNumber + ' ' + totalBeats + ' ' + totalTime + ' ' + position + ' ' + debugInfo);
            // console.log(position)
            console.log(debugInfo)
            // console.log(hurusato.slice(debugInfo.ev.startChar, debugInfo.ev.endChar) + ' ' + debugInfo.endMs)
            // 楽曲が終了したとき
    ...
    function eventCallback(ev) {
        ...
        if (ev) {
            ...
            // console.log(ev.midiPitches['pitch'])
